...
import os
from plotting_function import process_contrast, brain_plotting, brain_plotting_left
import pandas as pd

measurements = ['FC'] # 'Hurst', 'FC', 'Edges'
...
for m, c in product(measurements, contrasts):
    ...
    u1_vals, min_loading, max_loading, sigs = process_contrast(m, c, reverse=rev, keep=keep)
    ...
    brain_plotting(u1_vals, f'{m}_{c}', min_loading, max_loading, col)
    brain_plotting_left(u1_vals, f'{m}_{c}_left', min_loading, max_loading, col)
    plt.clf()
